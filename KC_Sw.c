...
unsigned int SwSession_Key[KEY_LEN] = { 0x833D3433, 0x229F389F, 0x2398E64F, 0x417ACF39 };//master key as hex (ES to Switch)
unsigned int SwSession_next_Key[KEY_LEN] = { 0x22F389F,0x833D3433,  0x417ACF39, 0x2398E64F };//master key as hex (ES to Switch_next key
unsigned int EsSession_Key[KEY_LEN] = { 0x3433833D, 0x389F009F, 0xE64F2398, 0xCF39417A };//master key as hex (Switch to ES)
unsigned int* curr_SwSession_Key;//Pointer to current session key
unsigned int* next_SwSession_Key;//Pointer to next session key
unsigned int* old_SwSession_Key;//Pointer to old session key
unsigned char sessionKey[32] = "833D3433229F389F2398E64F417ACF39";//master key as hex
unsigned char session2Key[32] = "3433833D389F229FE64F2398CF39417A";//master key as hex
unsigned char sessionKey_next[32] = "22F389F833D34332398E64F417ACF39";//master key as hex
...
void handleMsg(u_char *Uselesspointr, const struct pcap_pkthdr *header, const u_char *in_packet)
{
	...
	switch (msgFlag[0])
	{
		...
		if (incomingToggleBit[0] != toggleBit[0])
		{
			...
			//update key pointer
			old_SwSession_Key = &SwSession_Key;
			curr_SwSession_Key = &SwSession_next_Key;
			subkeys(chaskeySubkey1, chaskeySubkey2, curr_SwSession_Key);//Generate subkeys
		...
		if ((0 == memcmp((char*)hashValue, (char*)integrityVal, HASH_LEN)))
		{
			...
			chaskeyMsgLen = 444;
			counter = 0;
			chaskey2(hash, plaintext, EsSession_Key, chaskey1Subkey1, chaskey2Subkey2);//pointer to returned chasekey mac calculation
			printf("\n\nRe-calculated MIC: \n");	
			for (getData = 0; getData < hashLen; getData++)
			{
				printf("%02x", hash[getData]);
...
void main()
{
	...
	toggleVal = 0;//set first toggle bit
	
	//Generate subkeys
	curr_SwSession_Key = &SwSession_Key;
	next_SwSession_Key = &SwSession_next_Key;
	subkeys(chaskeySubkey1, chaskeySubkey2, curr_SwSession_Key);//call to key schedule function
