...
void handle_incoming(u_char *Uselesspointr, const struct pcap_pkthdr *header, const u_char *in_packet)
{
    ...
    newDigest = chaskey(hash, plaintext, ESSession_Key, chaskeySubkey1, chaskeySubkey2);//pointer to returned chasekey mac calculation
...
void regularUsage()
{
    ...
    pcap_close(devChannel);//close channel on which packets are sent
}////end_REGULAR_USAGE
/////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////
//---------------------------------------------------------------------------------------
...
void handleKE_Msg3(u_char *Uselesspointr, const struct pcap_pkthdr *header, const u_char *in_packet)
{
	...
	appendData = 0;
	for (getData = 48; getData < RANDOM_NUM_LEN; getData++)
	{
	...
	AES_CBC_decrypt_buffer(&ctx, incoming_payload, AES_BLOCK);
		...
		//otherwise --> close channel
		printf("Random number mismatch error\n");
		kdf_failure++;//Increment error count
		pcap_close(outChannel);//Close channel OR //start over??*********************************************************************************************************************************
	}//end_IF_ELSE
	...
	if (checkData >= 0)
	{
		printf("\nNo errors...generating session keys\n");
