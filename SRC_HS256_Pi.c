...
#include<string.h>//For various functions for manipulating arrays of characters
...
void main()
{
    ...
    int BAG = 6;//Minimum time gap between successive messages
...
//Port 2 (Eth0.204)
    ...
    do {
        ...
        //Calculate hash
        digest = HMAC_SHA256(plaintext, secret_Key);
        memcpy(messageDigest, digest, 32);//Copy hash to message digest array
