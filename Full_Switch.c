...
#define KEY_EST_MSG1_LEN 52 //Length of KEY EST MSG 1 packet
#define KEY_EST_MSG2_LEN 106 //Length of KEY EST MSG 2 packet
#define KEY_EST_MSG3_LEN 106 //Length of KEY EST MSG 3 packet
...
#define RANDOM_NUM_LEN 10 //Length of random number for key establishment
...
unsigned char random_numberES_prime[RANDOM_NUM_LEN];
unsigned char random_numberSw_prime[RANDOM_NUM_LEN];

//For loops
int appendRandomNumber;//Used in FOR loop to append random number to Key Establishment
int getPayload;//Used in FOR loop to get packet payload
int getRandomNumber;//Used in FOR loop to generate random number for Key Establishment

//Misc.
int kdf_failure;//Error count for Key Derivation
//---------------------------------------------------------------------------------------
//                AES FUNCTIONS
//Author: kokke
...
//---------------------------------------------------------------------------------------
//                HANDLING KEY DERIVATION MESSAGE 3 FROM ES
                    //1) Extract encrypted payload
                    //2) Decrypt payload
                    //3) Parse payload to retrieve R[ES]||R[sw]||F[sw]||Text[2])
                    //3) Initiate key derivation function if the random numbers are unchanged
//---------------------------------------------------------------------------------------
/*void handleKDF_Msg3(u_char *Uselesspointr, const struct pcap_pkthdr *header, const u_char *in_packet)
{
    struct ethernetHeader *ethdr = NULL;//Initialize struct
    struct ipheader *v4hdr = NULL;//Initialize struct
    struct udpheaderMsg3 *udpMsg3 = NULL;//Initialize struct
    ...
    udpMsg3 = (struct udpheaderMsg3*)(in_packet + SIZE_ETHERNET + SIZE_IP);//UDP header offset
    keyEST_mgs3 = (u_char *)(in_packet + SIZE_ETHERNET + SIZE_IP + SIZE_UDP);//Challenge offset

    //Retrieve message 2 payload
    for (getPayload = OFFSET; getPayload < MSG3_CONCAT_LEN; getPayload++)
    {
        msg23_concat[getPayload] = keyEST_mgs3[getPayload];//Fill payload array for decryption
    }//endFOR

    //call decryption program
    AES_init_ctx_iv(&ctx, ES204_masterKey, iv);
    AES_CBC_decrypt_buffer(&ctx, msg3_concat, AES_BLOCK);
    //Parse payload for: ////***************************************************************************************************************HERE
        //(1) R(ES) --> random_numberES_prime[RANDOM_NUM_LEN]
        //(2) R(switch) --> random_numberSw_prime[RANDOM_NUM_LEN]
        //(3) Identifier --> kdf_identifier[KEY_ID_LEN]
        //(4) F(ES) --> key_est_keyingMaterial[KEYING_MAT_LEN]
    
    //Compare R(switch) and R(switch)', and R(ES) and R(ES)'
    if (0 == memcmp((char*)random_numberSw (char*)random_numberSw_prime, CHALLENGE_LEN)) && (0 == memcmp((char*)random_numberES (char*)random_numberES_prime, CHALLENGE_LEN))
    {
...
//---------------------------------------------------------------------------------------
//                SECOND KDF MESSAGE TO ES
                //1) The Switch sends E[masterkey](R[sw]||R[ES]||I[ES]||F[sw]||Text[1])
//---------------------------------------------------------------------------------------
/*void KDF_secondMessage()
{
    struct AES_ctx ctx;//Initialize struct
    ...
    msg2_packet[27] = (0xa8);
    msg2_packet[28] = (0xb2);
    msg2_packet[29] = (0x5c);//PC
        //dst_ip
    msg2_packet[30] = (0xc0);
    msg2_packet[31] = (0xa8);
    msg2_packet[32] = (0xb2);
    msg2_packet[33] = (0x5a);//Windows
        //src_port
    msg2_packet[34] = (0x07);
    msg2_packet[35] = (0xd0);
    //dst_port
    ...
    //Generate parameters for key establishment
    //Generate and concatenate parameters
    appendParameter = 0;
    for (getParameter = 0; getParameter < RANDOM_NUM_LEN; getParameter++)
    {
        msg2_concat[appendParameter] = random_numberES[getParameter];
        appendParameter++;
    }//endFOR
    appendParameter = 8;
    for (getParameter = 0; getParameter < IDENTIFIER_LEN; getParameter++)
    {
        identifer_ES[getParameter] = hex_digits[(rand() % 256)];
        msg2_concat[appendParameter] = random_numberSw[getParameter];
        appendParameter++;
    }//endFOR
    appendParameter = 16;
    for (getParameter = 0; getParameter < KEYING_MAT_LEN; getParameter++)
    {
        key_est_keyingMaterial[getParameter] = hex_digits[(rand() % 256)];
        msg2_concat[appendParameter] = random_numberSw[getParameter];
        appendParameter++;
    }//endFOR
    appendParameter = 24;
    for (getParameter = 0; getParameter < DATA_FIELD_LEN; getParameter++)
    {
        key_est_data[getParameter] = hex_digits[(rand() % 256)];
        msg2_concat[appendParameter] = random_numberSw[getParameter];
        appendParameter++;
    }//endFOR
    ...
    for(appendParameter = 0; appendParameter < MSG2_CONCAT_LEN; appendParameter++)
    {
        msg2_packet[getParameter] = msg2_concat[appendParameter];
        getParameter++;
    }//end_FOR

    //send KDF message 2
    pcap_sendpacket(Channel204, msg2_packet, KEY_EST_MSG2_LEN);
    
    //Handle KDF message 3 then start key generation
    pcap_loop(Channel204, NEXT_INCOMING, verifyMessage3, NULL);//Start packet capture on Ethernet port

}//end_KDF_SECOND_MESSAGE*/
//---------------------------------------------------------------------------------------
//                HANDLING KEY DERIVATION MESSAGE 1 FROM ES
                    //1)Read message from ES
                    //2)Extract R(ES)
                    //2)ES sends E[masterkey](R[ES]||R[sw]||F[sw]||Text[2])
                    //3)Initiate key derivation function
//---------------------------------------------------------------------------------------
void handleKDF_Msg1(u_char *Uselesspointr, const struct pcap_pkthdr *header, const u_char *in_packet)
{
    struct ethernetHeader *ethdr = NULL;//Initialize struct
    struct ipheader *v4hdr = NULL;//Initialize struct
    ...
    {
        msg_packet1[getPayload] = random_numberES[getPayload];//Fill payload array for decryption
...
//        pcap_loop(Channel203, PACKET_COUNT, packetHandler, NULL);//Start packet capture on port 1
        pcap_loop(Channel204, NEXT_INCOMING, handleKDF_Msg1, NULL);//Start packet capture on port 2
}//endOPEN_INTERFACES
//---------------------------------------------------------------------------------------
...
void main()
{
    formatMasterKeys();//Convert from character stream to hex

///call other functions to start communication
