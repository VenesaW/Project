...
#define CHALLENGE_REQUEST_LEN 170 //Length of CHALLENGE REQUEST packet
...
#define KEY_LEN 32 //Key length of secret key (128 bits)
...
//---------------------------------------------------------------------------------------
//For keying material
unsigned char ES201_masterKey[] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22};//master key (encryption and decryption)
unsigned char ES202_masterKey[] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22};//master key (encryption and decryption)
unsigned char ES203_masterKey[] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22};//master key (encryption and decryption)
unsigned char ES204_masterKey[] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22};//master key (encryption and decryption)
...
void challengeRequestPacket (){
	...
	crq_packet[27] = (0xa8);
	crq_packet[28] = (0xb2);
	crq_packet[29] = (0x5a);//Pi
		//dst_ip
	crq_packet[30] = (0xc0);
	crq_packet[31] = (0xa8);
	crq_packet[32] = (0xb2);
	crq_packet[33] = (0x5c);//Windows
		//src_port
	crq_packet[34] = (0x07);
	crq_packet[35] = (0xd0);
	//dst_port
...
void initializationHandler(u_char *Uselesspointr, const struct pcap_pkthdr *header, const u_char *in_packet)
{
	...
	if (0 == memcmp((char*)ES201_identifier, (char*)the_identifier, KEY_ID_LEN))
	{
		...
		pcap_sendpacket(Channel201, ini_packet, INI_PACKET_LEN);//Challenge request packet
		...
		AES_CBC_encrypt_buffer(&ctx, the_request, AES_BLOCK);
		challengeRequestPacket();//Generate packet
		pcap_sendpacket(Channel202, ini_packet, INI_PACKET_LEN);//Challenge request packet
		...
		AES_CBC_encrypt_buffer(&ctx, the_request, AES_BLOCK);
		challengeRequestPacket();//Generate packet
		pcap_sendpacket(Channel203, ini_packet, INI_PACKET_LEN);//Challenge request packet
		...
		AES_CBC_encrypt_buffer(&ctx, the_request, AES_BLOCK);
		challengeRequestPacket();//Generate packet
		pcap_sendpacket(Channel204, ini_packet, INI_PACKET_LEN);//Challenge request packet
	} else {
