...
unsigned char Sw_RandomNum[RANDOM_NUM_LEN] = "AABBCCDD";//8 Byte random number (Nonce) for key establishment] = "AA112233";//Random number for Key Establishment message 1
...
unsigned char Sw_keyMat[KEYING_MAT_LEN] = "FEDCBA0123456789";//Keying material for Key Establishment message 3
unsigned char Sw_Nonce[NONCE_LEN] = "11234567";//Nonce for Key Establishment message 3
...
void KE_failureMsg()
{
	...
	{
		msg7_packet[appendData] = failureMSG[getData];
		appendData++;
	}//endFOR
	...
	//send packet
	pcap_sendpacket(Channel204, msg7_packet, KEY_EST_MSG67_LEN);//KDF message 5 packet
}//end_KE_FAILURE_MESSAGE
//////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////
//---------------------------------------------------------------------------------------
//                SUCCESS/FAILURE KDF MESSAGE TO Switch
...
void KE_successMsg()
{
	...
	for (getData = 0; getData < ANSWER_LEN; getData++)
	{
		msg6_packet[appendData] = successfulMSG[getData];
		appendData++;
	}//endFOR
	for (getData = 0; getData < CHALLENGE_LEN; getData++)
	{
	...
	//send packet
	pcap_sendpacket(Channel204, msg6_packet, KEY_EST_MSG67_LEN);//KDF message 5 packet
}//end_KE_SUCCESS_MESSAGE
//////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////
//---------------------------------------------------------------------------------------
//                HANDLING KEY DERIVATION MESSAGE 5 FROM Switch
					//1)Extract challenge
					//2)Create hash
//---------------------------------------------------------------------------------------
void handleKE_Msg5(u_char *Uselesspointr, const struct pcap_pkthdr *header, const u_char *in_packet)
{
	...
	if ((0 == memcmp((char*)ES_challengeResponse, (char*)Sw_challengeHash, HASH_LEN)))
	{
		KE_successMsg();
	}
	else {
...
void KE_fourthMessage()
{
	...
	AES_CBC_encrypt_buffer(&ctx, Sw_challenge, AES_BLOCK);
	...
	pcap_loop(Channel204, NEXT_INCOMING, handleKE_Msg5, NULL);//Start packet capture on port 2
...
//                HANDLING KEY DERIVATION MESSAGE 4 FROM Switch
					...
					//2)Create hash
//---------------------------------------------------------------------------------------
void handleKE_Msg3(u_char *Uselesspointr, const struct pcap_pkthdr *header, const u_char *in_packet)
{
	struct ethernetHeader *ethdr = NULL;//Initialize struct
	struct ipheader *v4hdr = NULL;//Initialize struct
	struct udpheader *udpMsg2 = NULL;//Initialize struct
	...
	for (getData = OFFSET; getData < MSG3_ENC_LEN; getData++)
	{
		incoming_payload[getData] = ES_payload[getData];//Fill payload array for decryption
	}//endFOR
	...
	//Compare R(ES) == R(ES)
	if ((0 == memcmp((char*)integrity_payload, (char*)Sw_RandomNum, RANDOM_NUM_LEN)))
	{
		//decrypt message
		//Call decryption program
	AES_init_ctx_iv(&ctx, SwMaster_Key, iv);
	...
	//Parse payload for:
	appendData = 0;
	///(1) I(Sw) --> Sw identifier
	for (getData = 0; getData < IDENTIFIER_LEN; getData++)
	{
		ES_swID[getData] = incoming_payload[appendData];
		appendData++;
	}
	///(2) F(ES) --> Keying material
	for (getData = 0; getData < KEYING_MAT_LEN; getData++)
	{
		ES_keyMat[getData] = incoming_payload[appendData];
		appendData++;
	}
	///(3) N(ES) --> Nonce
	for (getData = 0; getData < NONCE_LEN; getData++)
	{
		ES_Nonce[getData] = incoming_payload[appendData];
		appendData++;
	}
...
void handleKE_Msg1(u_char *Uselesspointr, const struct pcap_pkthdr *header, const u_char *in_packet)
{
	...
	for (getData = OFFSET; getData < RANDOM_NUM_LEN; getData++)
	{
		incoming_payload[getData] = ES_payload[getData];//Fill payload array for decryption
	}//endFOR
...
void main()
{
	...
	//pcap_loop(Channel203, PACKET_COUNT, packetHandler, NULL);//Start packet capture on port 1
        pcap_loop(Channel204, NEXT_INCOMING, handleKE_Msg1, NULL);//Start packet capture on port 2
